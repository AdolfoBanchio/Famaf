...
static bool invrep(graph_t g) {
    bool inv;
    inv = g!=NULL && g->costs!=NULL; /* la estructura basica no puede tener puntero a null */
    for (unsigned int i = 0; i < g->max_vertexs; i++)
    {
        inv = inv && g->costs[i]!=NULL; /* cada puntero dentro del arreglo principal no debe ser null  */
    }
    return inv;
}
